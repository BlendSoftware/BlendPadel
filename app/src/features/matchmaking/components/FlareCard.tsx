import { MapPin, Clock, MessageCircle, ArrowRight, Zap } from 'lucide-react'
...
export function FlareCard({ flare, onRespond, isResponding = false }: FlareCardProps) {
  ...
  const hasLoc     = (flare.lat ?? flare.latitude) !== null && (flare.lat ?? flare.latitude) !== 0
    ...
    <article className={styles.card}>
      ...
      {flare.message && (
        <p className={styles.message}>
          <MessageCircle size={12} style={{ display: 'inline', marginRight: 4, opacity: 0.4 }} />
          {flare.message}
        </p>
      )}
