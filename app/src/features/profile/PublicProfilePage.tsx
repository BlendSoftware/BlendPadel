...
export function PublicProfilePage() {
  ...
  const fullName = [publicProfile.name, publicProfile.last_name].filter(Boolean).join(' ').trim()
      ...
      <div className="px-4 pt-6 pb-6 space-y-4">
        ...
        <Card>
          <div className="flex flex-col items-center gap-3 py-2">
            ...
            {/* Trust label computed from trust_score */}
            <Badge variant={trustVariant(trustLabelFromScore(publicProfile.trust_score))}>
              Confianza: {trustLabelFromScore(publicProfile.trust_score)}
            </Badge>
          </div>
        </Card>
