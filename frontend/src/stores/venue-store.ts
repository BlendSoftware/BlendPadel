...
export const useVenueStore = create<VenueState & VenueActions>((set) => ({
  ...
  updateVenue: async (id, data) => {
    set({ isLoading: true, error: null })
    try {
      const res = await api.put<Venue>(`/venues/${id}`, data)
      const updated = res.data
      set((s) => ({
        venues: s.venues.map((v) => (v.id === id ? updated : v)),
        selectedVenue: s.selectedVenue?.id === id ? updated : s.selectedVenue,
        isLoading: false,
      }))
      return updated
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : 'Error al actualizar cancha'
      set({ error: message, isLoading: false })
      throw e
    }
  },
