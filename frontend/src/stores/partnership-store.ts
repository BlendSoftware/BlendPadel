...
export interface SentPartnershipRequest {
  id: string
  status: 'pending' | 'accepted' | 'rejected' | 'dissolved'
  requested_user: { id: string; full_name: string }
  created_at: string
}

interface PartnershipState {
  partnerships: Partnership[]
  sentRequests: SentPartnershipRequest[]
...
export const usePartnershipStore = create<PartnershipState & PartnershipActions>((set) => ({
  ...
  fetchSentRequests: async () => {
    try {
      const res = await api.get<{ requests: SentPartnershipRequest[] }>('/partnerships/requests/sent')
      set({ sentRequests: res.data.requests ?? [] })
    } catch (e: unknown) {
      const message = extractApiError(e, 'Error al cargar solicitudes enviadas')
      set({ error: message })
    }
  },
